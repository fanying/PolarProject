...
def add2CurrentRaster(filename, folder = 'modisProcessing/SAR/tiff/'):
    ...
    for dripath, dirnames, filenames in os.walk(workspace):
        for workspacefilename in filenames:
            if workspacefilename.endswith('_CURRENT_RASTER_%d.tif' %RESOLUTION):
    ...
    if not tiff_list:
        ...
        print('add2CurrentRaster',geo_t)
        # exit()
        new_band = dataset.GetRasterBand(1)
        new_array = new_band.ReadAsArray()
        print('-----------------------min(new_array)', np.min(new_array))
        ...
        new_x_size = math.ceil((new_lrx-new_ulx)/RESOLUTION)
        new_y_size = math.ceil((new_uly-new_lry)/RESOLUTION)
        drv = gdal.GetDriverByName('GTiff')
        dest = drv.Create(workspace+str(next_no)+'_CURRENT_RASTER_%d.tif' %RESOLUTION, int(new_x_size), int(new_y_size), 1, gdal.GDT_Byte)
        new_geo = (new_ulx, RESOLUTION, 0.0, new_uly, 0.0, -RESOLUTION)
        print new_geo
        ...
        next_no = curr_no + 1
        curr_dataset = gdal.Open(workspace+str(curr_no)+'_CURRENT_RASTER_%d.tif' %RESOLUTION, GA_ReadOnly)
        ...
        print('else add2CurrentRaster', curr_geo_t)
        # exit()
        ...
        x_size = math.ceil((lrx - ulx)/RESOLUTION)
        y_size = math.ceil((uly - lry)/RESOLUTION)
        print (x_size, y_size)

        drv = gdal.GetDriverByName('GTiff')
        dest = drv.Create(workspace + str(next_no) + '_CURRENT_RASTER_%d.tif' %RESOLUTION, int(x_size), int(y_size), 1, gdal.GDT_Byte)
        geo = (ulx, RESOLUTION, 0.0, uly, 0.0, -RESOLUTION)
        print geo
        ...
        array = band.ReadAsArray()
        array[array==0] = 255
        new_start_x = int((new_ulx - ulx)/RESOLUTION)
        new_start_y = int((uly - new_uly)/RESOLUTION)
        print('new_start_x', new_start_x)
    ...
    # dataset = gdal.Open(workspace + str(next_no) + '_CURRENT_RASTER_%d.tif' %RESOLUTION, GA_ReadOnly)
    # band = dataset.GetRasterBand(1)
    # array = band.ReadAsArray()
    # print('array.shape', array.shape)
    # tifimg = Image.fromarray(array)
    # tifimg.save(workspace + str(next_no) + '_CURRENT_RASTER_%d.jpg' %RESOLUTION)

    return str(next_no) + '_CURRENT_RASTER_%d' %RESOLUTION


def getLonLat(filename, folder = 'modisProcessing/SAR/tiff/arcmapWorkspace/'):
    ...
    # new_y_size = int(math.floor(y_size))
    # new_x_size = int(math.floor(x_size))
    print new_x_size, new_y_size
    ...
    for i in range(new_x_size):
        for j in range(new_y_size):
            ...
            # (lonlats[j, i, 0] ,lonlats[j, i, 1], tmp) = tx.TransformPoint(geo_t[0]+geo_t[1]*i, geo_t[3]+geo_t[5]*j)
            (lonlats[j, i, 0], lonlats[j, i, 1], tmp) = tx.TransformPoint(geo_t[0] + geo_t[1] * (i * 20 + 10),
                                                                  geo_t[3] + geo_t[5] * (j * 20 + 10))

    # scipy.io.savemat(folder+filename+ '_lonlat.mat', {'array':lonlats})
    # file = open(folder+filename+'.lonlat', 'wb')
    # pickle.dump(lonlats, file, protocol=2)
    # file.close()
    np.save(folder+filename + '_lonlat.npy', lonlats)

def getProb(tiff_name, prob_folder = 'modisProcessing/SAR/Proba/', ice_folder = 'modisProcessing/SAR/Ice/', tiff_folder = 'modisProcessing/SAR/tiff/arcmapWorkspace/'):

    file_list = []
    for dripath, dirnames, filenames in os.walk(prob_folder):
        for filename in filenames:
            if not filename.startswith('.'):
                print('in getProb: filename', filename)
                file_info = []
                # print filename
    ...
    # print('file_info[3] -file_info[6]*file_info[4]', file_info[3] -file_info[6]*file_info[4])
    # print(geo_t[3]+geo_t[5]*((new_y_size - 1)*20+10))
    # print('file_info[2]+file_info[5]*file_info[4]',file_info[2]+file_info[5]*file_info[4])
    # print('geo_t[0]+geo_t[1]*(i*20+10)', geo_t[0]+geo_t[1]*((new_x_size - 1)*20+10))
                ...
                for k in range(len(file_list)):
                    ...
                    if x >= file_info[2] and y <= file_info[3] and x <= file_info[2]+file_info[5]*file_info[4] and y >= file_info[3] \
                        -file_info[6]*file_info[4]:
                            ...
                            # print i1, j1
                            if i1 < prob_results.shape[1] and j1 < prob_results.shape[0]  and not np.isnan(prob_results[j1, i1, 0]) and not np.isnan(ice_results[j1, i1]):
    ...
    for i in range(new_x_size):
        for j in range(new_y_size):
            if np.isnan(probs[j, i, 2]):
                if j > 1 and j < new_y_size - 1 and  not np.isnan(probs[j - 1, i, 2]) and not np.isnan(probs[j + 1, i, 2]):
                    # print('j getProb~~~~~~~~~~~~~~~~~~~~~~~~~~:',i,j)
                    probs[j, i, 2] = (probs[j - 1, i, 2] + probs[j + 1, i, 2]) / 2.0
                elif  i > 1 and i < new_x_size - 1 and  not np.isnan(probs[j, i - 1, 2]) and not np.isnan(probs[j, i + 1, 2]):
                    probs[j, i, 2] = (probs[j, i - 1, 2] + probs[j, i + 1, 2]) / 2.0
                    # print('i getProb~~~~~~~~~~~~~~~~~~~~~~~~~~:', i, j)
    ...
    #                         # probs[j, i, 2] = 1
    #                         # ices[j, i] = 1
    #                         if j1 < prob_results.shape[1]:
    #                             # probs[j, i, 0] = prob_results[j1 - 1, i1, 0]
    #                             # probs[j, i, 1] = prob_results[j1 - 1, i1, 1]
    #                             # probs[j, i, 2] = prob_results[j1 - 1, i1, 2]
    #                             # ices[j, i] = ice_results[j1 - 1, i1]
    #                             probs[j, i, 0] = prob_results[j1, i1, 0]
    #                             probs[j, i, 1] = prob_results[j1, i1, 1]
    #                             probs[j, i, 2] = prob_results[j1, i1, 2]
    #                             ices[j, i] = ice_results[j1, i1]
    ...
    #                         #     probs[j, i, 2] = prob_results[j1, i1, 2]
    #                         #     ices[j, i] = ice_results[j1, i1]
    #                             # if i1 == prob_results.shape[1] - 1 or np.isnan(prob_results[j1, i1 + 1, 0]):
    #                             #     # if x + geo_t[1] * 10 >= file_info[2]+file_info[5]*file_info[4] :
    #                             #     print('i:', i)
    #                             #     print('j:', i)
    #                             #     print('i1:', i1)
    #                             #     print('j1:', i1)
    #                             #     print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!in xxxxxxxxxxxxxxxxxxxxxxxxxxxx')
    #                             #     probs[j, i + 1, 0] = prob_results[j1, i1, 0]
    #                             #     probs[j, i + 1, 1] = prob_results[j1, i1, 1]
    #                             #     probs[j, i + 1, 2] = prob_results[j1, i1, 2]
    #                             #     probs[j, i + 2, 0] = prob_results[j1, i1, 0]
    #                             #     probs[j, i + 2, 1] = prob_results[j1, i1, 1]
    #                             #     probs[j, i + 2, 2] = prob_results[j1, i1, 2]
    #                             # if j1 == prob_results.shape[0] - 1 or np.isnan(prob_results[j1 + 1, i1, 0]):
    #                             # if j1 == prob_results.shape[0] - 2:
    #                                 # if y + geo_t[5] * 10 >= file_info[3]:
    #                                 # print('i:', i)
    #                                 # print('j:', i)
    #                                 # print('i1:', i1)
    #                                 # print('j1:', i1)
    #                                 # print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!in yyyyyyyyyyyyyyyyyyyyyyyyyyyyy')
    #                                 # probs[j + 1, i, 0] = prob_results[j1 + 1, i1, 0]
    #                                 # probs[j + 1, i, 1] = prob_results[j1 + 1, i1, 1]
    #                                 # probs[j + 1, i, 2] = prob_results[j1 + 1, i1, 2]
    #                                 # probs[j + 2, i, 0] = prob_results[j1, i1, 0]
    #                                 # probs[j + 2, i, 1] = prob_results[j1, i1, 1]
    #                                 # probs[j + 2, i, 2] = prob_results[j1, i1, 2]
    #                             # break
    # print probs
    # print probs.shape
    # print probs.dtype
    ...
    # scipy.io.savemat(tiff_folder+tiff_name+'_prob.mat', {'array':probs})
    # scipy.io.savemat(tiff_folder + tiff_name + '_ice.mat', {'array':ices})
    np.save(tiff_folder + tiff_name + '_prob.npy', probs)
    np.save(tiff_folder + tiff_name + '_ice.npy', ices)


# def getProb_old(tiff_name, prob_folder = 'SAR/Proba/', ice_folder = 'SAR/Ice/', tiff_folder = 'SAR/tiff/arcmapWorkspace/'):
#
#     file_list = []
#     for dripath, dirnames, filenames in os.walk(prob_folder):
#         for filename in filenames:
...
#
#     dataset = gdal.Open(tiff_folder+tiff_name+'.tif', GA_ReadOnly)
#     print('tiff_folder+tiff_name', tiff_folder+tiff_name)
#     band = dataset.GetRasterBand(1)
#     array = band.ReadAsArray()
#     x_size = dataset.RasterXSize # Raster xsize
#     y_size = dataset.RasterYSize # Raster ysize
...
#     probs = np.ones((new_y_size, new_x_size, 3))
#     ices = np.ones((new_y_size, new_x_size))
#     prob_results = prob_results_list[0]
#     print('prob_results', prob_results.shape)
#     for i in range(new_x_size):
#         for j in range(new_y_size):
#             x = geo_t[0]+geo_t[1]*(i*20+10)
#             y = geo_t[3]+geo_t[5]*(j*20+10)
#             # x = geo_t[0] + geo_t[1] * (i * 20)
#             # y = geo_t[3] + geo_t[5] * (j * 20)
#             probs[j, i] = [np.nan, np.nan, np.nan]
#             ices[j, i] = 0
#             if array[j*20+8, i*20+8] != 255:
#             # if array[j * 20, i * 20] != 255:
#                 for k in range(len(file_list)):
#
#                     if x >= file_info[2] and y <= file_info[3] and x <= file_info[2]+file_info[5]*file_info[4] and y >= file_info[3] \
#                         -file_info[6]*file_info[4]:
...
#                             i1 = int(math.floor((x - file_info[2])/file_info[4]/20))
#                             j1 = int(math.floor((y - file_info[3])/(-file_info[4])/20))
#                             # i1 = int(math.ceil((x - file_info[2]) / file_info[4] / 20))
#                             # j1 = int(math.ceil((y - file_info[3]) / (-file_info[4]) / 20))
#                             # print i1, j1
#                             # if not np.isnan(prob_results[j1, i1, 0]) and not np.isnan(ice_results[j1, i1]):
#                             if not np.isnan(prob_results[j1, i1, 0]):
#                                 probs[j, i, 0] =  prob_results[j1, i1, 0]
#                                 probs[j, i, 1] = prob_results[j1, i1, 1]
#                                 probs[j, i, 2] = prob_results[j1, i1, 2]
#                                 ices[j, i] = ice_results[j1, i1]
...
#                                     print('j1:', i1)
#                                     print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!in yyyyyyyyyyyyyyyyyyyyyyyyyyyyy')
#                                     probs[j + 1, i, 0] = prob_results[j1, i1, 0]
#                                     probs[j + 1, i, 1] = prob_results[j1, i1, 1]
#                                     probs[j + 1, i, 2] = prob_results[j1, i1, 2]
#                                     probs[j + 2, i, 0] = prob_results[j1, i1, 0]
#                                     probs[j + 2, i, 1] = prob_results[j1, i1, 1]
#                                     probs[j + 2, i, 2] = prob_results[j1, i1, 2]
#                                 break
...
def putpixel(fname, folder = 'modisProcessing/SAR/tiff/arcmapWorkspace/'):
...
def getRGBfromProb(prob):
    ...
    # Rslipts = [0, 0, 0, 55, 155, 255, 255, 225, 180, 128, 120]
    # Gslipts = [0, 0, 0, 55, 155, 255, 50, 0, 0, 0, 0]
    # Bslipts = [80, 150, 220, 255, 255, 255, 50, 0, 0, 0, 0]
    # Bslipts = [80, 150, 220, 255, 255, 150, 100, 50, 0, 0, 0]
...
def cropandmask(ulx, uly, lrx, lry, fname, folder = 'modisProcessing/SAR/tiff/arcmapWorkspace/'):
    ...
    lonlatfile = folder + fname + '_lonlat.npy'
    probfile = folder + fname + '_prob.npy'
    icefile = folder + fname + '_ice.npy'
    ...
    # ulx = ulx - 200000 if ulx - 200000 > geo_t[0] else geo_t[0]
    # uly = uly + 200000 if uly + 200000 < geo_t[3] else geo_t[3]
    # lrx = lrx + 200000 if lrx + 200000 < geo_t[0] + geo_t[1] * x_size else geo_t[0] + geo_t[1] * x_size
    # lry = lry - 200000 if lry - 200000 > geo_t[3] + geo_t[5] * y_size else geo_t[3] + geo_t[5] * y_size
    ...
    # uli = int(np.floor(np.round((ulx - geo_t[0]) / geo_t[1])))
    # ulj = int(np.floor(np.round((uly - geo_t[3]) / geo_t[5])))
    # lri = int(np.floor(np.round((lrx - geo_t[0]) / geo_t[1])))
    # lrj = int(np.floor(np.round((lry - geo_t[3]) / geo_t[5])))
    # print (uli, ulj)
    # print (lri, lrj)
    ...
    lonlat = np.load(lonlatfile)
    prob = np.load(probfile)
    ice = np.load(icefile)

    # print('lonlatfile', lonlatfile)
    # print('icefile', icefile)
    # print('before loadmat')
    # exp = sio.loadmat(folder + 'example.mat')
    # exp = exp['a']
    # print('exp.shape', exp.shape)
    # ice = sio.loadmat(icefile)
    # ice = ice['array']
    # print('before loadmat 3')
    # lonlat = sio.loadmat(lonlatfile)
    # print('before loadmat 2')
    # lonlat = lonlat['array']
    # # print lonlat.shape
    # print('before loadmat 1')
    # prob = sio.loadmat(probfile)
    # prob = prob['array']
    ...
    crop_name = cu_time + '_CURRENT_RASTER_%d' %RESOLUTION
    # print jpg.size
    band = dataset.GetRasterBand(1)
    array = band.ReadAsArray()
    array_crop = np.copy(array[ulj:lrj, uli:lri])
    jpg_crop = Image.fromarray(array_crop)
    # print jpg_crop.size
    jpg_crop.save(folder + crop_name + '_crop.jpg')

    lonlat_crop = lonlat[ulj/20:lrj/20, uli/20:lri/20, :]
    # lonlat_crop = lonlat[ulj:lrj, uli:lri, :]
